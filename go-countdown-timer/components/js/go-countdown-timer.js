jQuery(function ($) {
	function go_countdown_timer_js()
	{
		// alert( "EHERE");
		$(".go_countdown_timer_event").each(function (i) {
	    ...
	    window.setTimeout( go_countdown_timer_js, 1000);
	}

	go_countdown_timer_js();
	...
	function process_date ( targetTime, nowTime ) 
	{
		...
		if( showMonth ){
			if(sigNumHit || !stripZero || (resultantMonth + parseInt(rollover/2628000)) ){
				...
				s = s + ( '<span class="go_countdown_timer_month go_countdown_timer_timeunit">' +  resultantMonth + ( resultantMonth > 1 ) ? resultantMonth + ' months ' : resultantMonth + ' month ' + '</span> ' );
		...
		*/
		...
		if( showDay ){
			if(sigNumHit || !stripZero || (resultantDay + parseInt(rollover/86400)) ){
				...
				s = s + ( '<span class="go_countdown_timer_day go_countdown_timer_timeunit">' + resultantDay + ( resultantDay > 1 ) ? resultantDay + ' days ' : resultantDay + ' day ' + '</span> ' );
		...
		if( showHour ){
			if(sigNumHit || !stripZero || (resultantHour + parseInt(rollover/3600)) ){
				...
				s = s + ( '<span class="go_countdown_timer_hour go_countdown_timer_timeunit">' + resultantHour + ( resultantHour > 1 ) ? resultantHour + ' hours ' : resultantHour + ' hour ' + '</span> ' );
		...
		if( showMinute ){
			if(sigNumHit || !stripZero || (resultantMinute + parseInt(rollover/60)) ){
				...
				s = s + ( '<span class="go_countdown_timer_minute go_countdown_timer_timeunit">' + resultantMinute + ( resultantMinute > 1 ) ? resultantMinute + ' minutes ' : resultantMinute + ' minute '  + '</span> ' );
		...
		if( showSecond ) {
			...
			s = s + ( '<span class="go_countdown_timer_second go_countdown_timer_timeunit">' + resultantSecond + ( resultantSecond > 1 ) ? resultantSecond + ' seconds ' : resultantSecond + ' second '   + '</span> ' );
		}
